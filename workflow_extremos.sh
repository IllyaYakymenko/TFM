...
ficheros="$(ls *.dat)"
echo $ficheros | wc -w >> extremos.in
echo $ficheros >> extremos.in

# Número de filas
fich1="$(echo $ficheros | awk '{print $1}')"
numfilas="$(echo $(wc -l $fich1))"
echo $numfilas| awk '{print $1}' >> extremos.in

# Número de columnas
head -n1 $fich1 | wc -w >> extremos.in

# Significancia
echo $4 >> extremos.in

./extremos.exe < extremos.in

mv fort.20 ../"Phi_y_Psi_extremos_$2_$3_registrado$4%.dat"
cd ..
